...
The narrative voice across the site — and by request, every piece of prose this design system generates — is that of a peer presenting reasoning to other capable, intellectually curious peers. Technical details are preserved and defined precisely on first reference; they are not translated into approximations. The reader is treated as intelligent and capable of following the argument without scaffolding. The full voice specification lives in `voice/style-card.md`, `voice/generative-patterns.md`, and `VOICE.md`.
...
├─ assets/
│   ├─ css/
│   │   └─ institutional.css    ← canonical styling engine and DOM structures
...
- **Fonts** are loaded from Google Fonts (Playfair Display, Lora, JetBrains Mono) — same as the live site. **No font files are bundled** because the live site itself relies on Google Fonts. If an offline build is ever needed, host the WOFF2 files locally and update `assets/css/institutional.css`.
